...
router.get("/rdv/:rdvId", async (req, res) => {
  try {
    ...
    // For clients: need valid token AND Google review left
    const hasValidToken = token && rdv.photosAccessToken === token;
    const canAccessPhotos = adminVerified || (hasValidToken && rdv.avisGoogleLeft);
